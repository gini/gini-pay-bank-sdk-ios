...
If you want to add the _File import_ feature on your app, first you need to specify the supported types (`fileImportSupportedTypes `) on the `GiniPayBankConfiguration` instance.

```swift
let giniPayBankConfiguration = GiniPayBankConfiguration()
giniPayBankConfiguration.fileImportSupportedTypes = .pdf_and_images
```
...
Also if you want to add some custom validations for the imported `GiniCaptureDocument`, you can specify them in the `GiniPayBankConfiguration` closure, `customDocumentValidations`. Here is an example:

```swift
giniPayBankConfiguration.customDocumentValidations = { document in
